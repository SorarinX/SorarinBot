...
func (a *Adapter) Start() error {
	...
	err := a.Bot.HotLogin(reload, ow.NewRetryLoginOption())
	if err == nil && a.onLoginSuccess != nil {
		// HotLogin bypasses LoginChecker, so fire callback manually
		a.onLoginSuccess()
	}
	return err
}
...
func (a *Adapter) onText(ctx *ow.MessageContext) {
	...
	// Dedup layer 2: content-based (catches cases where msgId differs)
	sender, err := ctx.Sender()
	if err != nil {
		logrus.Warnf("[text] failed to get sender: %v", err)
		return
	}
	if ctx.IsSendByGroup() {
		sender, err = ctx.SenderInGroup()
		if err != nil {
			logrus.Warnf("[text] failed to get group sender: %v", err)
			return
		}
	}
...
func (a *Adapter) onImage(ctx *ow.MessageContext) {
	...
	sender, err := ctx.Sender()
	if err != nil {
		logrus.Warnf("[image] failed to get sender: %v", err)
		return
	}
	if ctx.IsSendByGroup() {
		sender, err = ctx.SenderInGroup()
		if err != nil {
			logrus.Warnf("[image] failed to get group sender: %v", err)
			return
		}
	}
	nick := sender.NickName
