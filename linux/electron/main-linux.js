...
function startGoBackend() {
  const possiblePaths = [
    path.join(__dirname, 'SorarinBot'),
    path.join(process.resourcesPath || __dirname, 'SorarinBot'),
    path.join(path.dirname(process.execPath), 'SorarinBot'),
  ]
  ...
  if (!exePath) {
    console.error('[electron] SorarinBot not found in any of:', possiblePaths)
    return false
  }
...
app.whenReady().then(async () => {
  ...
  if (!started) {
    dialog.showErrorBox('SorarinBot', 'SorarinBot binary not found. Please reinstall.')
    app.quit()
    return
  }
  ...
  try {
    ...
    dialog.showErrorBox('SorarinBot', 'Backend startup timeout. Check if port ' + GO_PORT + ' is in use.')
    app.quit()
  }
})
